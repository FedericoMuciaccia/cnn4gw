...
for i in range(6):
    network = tflearn.layers.conv.conv_2d(network, nb_filter=9, filter_size=3, strides=1, padding='valid', activation='relu', bias=True, weights_init='uniform_scaling', bias_init='zeros', regularizer='L2', weight_decay=0.001) # activation='linear', regularizer=None, scope=None
...
network = tflearn.layers.core.fully_connected(network, n_units=number_of_classes, activation='softmax')
